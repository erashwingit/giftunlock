  ...
  return (
    <main className="min-h-screen bg-dark-900 text-white flex flex-col items-center justify-center px-4 py-16">
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 30%, rgba(239,68,68,0.05) 0%, transparent 70%)" }} />
      ...
      <div className="relative w-full max-w-md text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-black text-white">Memory Not Found</h1>
          <p className="text-sm leading-relaxed" style={{ color: "#4A4A58" }}>
            This memory link is invalid or expired. The link may have been
        ...
        >
          <Gift size={14} /> Create Your Own Memory
        </Link>
...
/* ─── Memory Page ────────────────────────────────────────── */
function MemoryPage({ order }: { order: Order }) {
  ...
  return (
    <main className="min-h-screen bg-dark-900 text-white">
      ...
      {/* Glow */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 60% 40% at 50% 20%, rgba(255,184,0,0.06) 0%, transparent 70%)" }} />

      <div className="relative max-w-lg mx-auto px-4 py-10 space-y-8">

        {/* Heading */}
        <div className="text-center space-y-2" style={{ animation: "fadeInUp 0.5s ease both" }}>
          <p className="text-xs font-bold uppercase tracking-widest" style={{ color: "#FFB800" }}>
            🎁 A memory was unlocked for you
          </p>
          <h1 className="text-3xl font-black text-white">
            {occasion ? `${occasion} Memory` : "Your Special Memory"}
          </h1>
          {order.product_type && (
        ...
        <div style={{ animation: "fadeInUp 0.5s ease 0.1s both" }}>
          {isReady ? (
            <div
              ...
              style={{ border: "1px solid rgba(255,184,0,0.2)", aspectRatio: "16/9" }}
            >
              <iframe
            ...
            >
              ...
              <div className="text-center px-4">
                <p className="font-bold text-white text-sm">Video Being Crafted</p>
                <p className="text-xs mt-1" style={{ color: "#4A4A58" }}>
                  Your cinematic memory is still being produced. Check back in 48 hours!
                </p>
              </div>
            </div>
          )}
        ...
        {photos.length > 0 && (
          <div style={{ animation: "fadeInUp 0.5s ease 0.3s both" }}>
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#FFB800" }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {photos.map((url, i) => (
                <div
                  ...
                  style={{ aspectRatio: "1/1", border: "1px solid rgba(255,184,0,0.1)" }}
                >
        ...
        {/* CTA */}
        <div className="text-center pt-2" style={{ animation: "fadeInUp 0.5s ease 0.4s both" }}>
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all hover:scale-[1.02]"
            style={{ background: "linear-gradient(135deg, #FFD700, #FF9A3C)", color: "#0A0A0B" }}
...
export default async function SquadPage({
  ...
  /* Fetch the order from Supabase by secure_slug */
  ...
  return <MemoryPage order={order as Order} />;
}
...
export async function generateMetadata({
  ...
  if (!order) {
    return {
      title: "Memory Not Found — GiftUnlock.in",
      description: "This memory link is invalid or expired.",
    };
  }

  return {
    title: `${order.occasion ?? "Special"} Memory — GiftUnlock.in`,
    description: `A personalised ${order.product_type} memory created with love on GiftUnlock.in`,
    openGraph: {
      title: `${order.occasion ?? "Special"} Memory — GiftUnlock.in`,
      description: "Unlock a cinematic memory crafted just for you.",
    },
  };
}
